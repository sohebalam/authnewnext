...
const Cart = () => {
    ...
    if (cartLocal && cartLocal.length > 0) {
      ...
      const updateCart = async () => {
        for (const item of cartLocal) {
          const res = await fetch(`${baseUrl}/product/${item._id}`)
          const { _id, title, selectedFile, price, description } = res
          if (inStock > 0) {
            newArr.push({
              _id,
              title,
  ...
  const handlePayment = async () => {
    ...
    if (newCart.length < cart.length) {
      ...
      console.log("The product is out of stock or the quantity is insufficient")
